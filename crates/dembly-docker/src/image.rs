...
pub fn image_create_command(plan: &ImageRuntimePlan) -> Vec<OsString> {
    let mut command = vec![
        ...
        "create".into(),
        "--name".into(),
        plan.container_name.clone().into(),
        "--privileged".into(),
    ];
    ...
    for card in &plan.cards {
        add_bind(
            &mut command,
            &card.source,
            &format!("/run/dembly/cards/{}.squashfs", card.name),
            true,
        );
    }
    ...
    command.extend([
        ...
        "0:0".into(),
        "--entrypoint".into(),
        "/run/dembly/bin/dembly".into(),
        plan.image.clone().into(),
        "__runtime".into(),
        "init".into(),
        "/run/dembly/runtime.toml".into(),
    ]);
    command
}
...
fn add_bind(command: &mut Vec<OsString>, source: &std::path::Path, target: &str, read_only: bool) {
    ...
    command.extend([
        "--volume".into(),
        format!("{}:{target}:{mode}", source.display()).into(),
    ]);
}
